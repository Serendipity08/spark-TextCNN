...
# 解析命令行参数
parser = argparse.ArgumentParser(description='Spark文本分类实现 - QNLI数据集')
parser.add_argument('--data_file', type=str, default='hdfs://hadoop1:9000/user/spark/glue/QNLI/train.tsv', help='训练数据文件路径')
parser.add_argument('--test_file', type=str, default='hdfs://hadoop1:9000/user/spark/glue/QNLI/dev.tsv', help='测试数据文件路径')
...
parser.add_argument('--output_fig', type=str, default='qnli_model_comparison.png', help='输出图表文件名')
...
# 加载QNLI数据集
def load_qnli_data(file_path, is_test=False):
    ...
    try:
        ...
        # 重命名列
        data = data.withColumnRenamed("question", "question") \
            .withColumnRenamed("sentence", "sentence")

        # 如果不是测试集且有label列，处理标签
        if not is_test and "label" in data.columns:
            # 将标签转换为数值
            data = data.withColumn("label",
                                   when(col("label") == "entailment", lit(1.0))
                                   .when(col("label") == "not_entailment", lit(0.0))
        ...
        # 合并问题和句子为一个文本字段
        data = data.withColumn("text", concat_ws(" [SEP] ", col("question"), col("sentence")))
        ...
        # 创建一个简单的示例数据集
        data = [("What is the capital of France? [SEP] Paris is the capital of France.", 1.0),
                ("Who invented the telephone? [SEP] The telephone was invented by Alexander Graham Bell.", 1.0),
                ("What is the boiling point of water? [SEP] Gold is a precious metal.", 0.0)]
...
def main():
    # 加载数据
    train_data = load_qnli_data(args.data_file)
    test_data = load_qnli_data(args.test_file)
